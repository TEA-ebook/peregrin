class Peregrin::Epub
  ...
  def root_to_xhtml(root)
    ...
    remove_namespaced_attributes(root, 'xml')
  ...
  class NotAZipArchive < ValidationError;
  end
  class FailureLoadingOCF < ValidationError;
  end
  class FailureLoadingOPF < ValidationError;
  end
  class FailureLoadingNCX < ValidationError;
  end
  class FailureLoadingNAV < ValidationError;
  end
